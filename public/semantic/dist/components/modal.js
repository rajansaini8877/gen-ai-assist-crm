      ...
      module = {
                ...
                $("<button/>", {
                  ...
                  on: {
                    click: function () {
                      var button = $(this);
                      if (
                        button.is(selector.approve) ||
                        button.is(selector.deny) ||
                        click.call(element, $module) === false
                      ) {
                        return;
                      }
                      module.hide();
        ...
        create: {
          modal: function () {
            ...
            if (settings.title !== "") {
              ...
              $("<div/>", { class: className.title, id: titleId }).appendTo(
                $module,
              );
            }
            if (settings.content !== "") {
        ...
        refreshInputs: function (ignoreAutofocus) {
          ...
          $inputs
            .first()
            .on(
              "keydown" + elementEventNamespace,
              module.event.inputKeyDown.first,
            );
          $inputs
            .last()
            .on(
              "keydown" + elementEventNamespace,
              module.event.inputKeyDown.last,
            );
          if (
            !ignoreAutofocus &&
            settings.autofocus &&
        ...
        bind: {
          events: function () {
            ...
            $closeIcon.on(
              "keyup" + elementEventNamespace,
              module.event.closeKeyUp,
            );
            $window
              .on("resize" + elementEventNamespace, module.event.resize)
              .on("focus" + elementEventNamespace, module.event.focus);
            $context.on("click" + elementEventNamespace, module.event.click);
          },
          scrollLock: function () {
            // touch events default to passive, due to changes in chrome to optimize mobile perf
            $dimmable[0].addEventListener(
              "touchmove",
        ...
        unbind: {
          scrollLock: function () {
            $dimmable[0].removeEventListener(
              "touchmove",
              module.event.preventScroll,
              { passive: false },
            );
        ...
        event: {
          ...
          click: function (event) {
            if (
              ...
              module.is.active() &&
              settings.autofocus &&
              $(document.activeElement).closest(selector.modal).length === 0
            ) {
              ...
              if (settings.allowMultiple) {
                if (module.others.active()) {
                  $otherModals
                    ...
                    .removeClass("out")
                    .addClass("transition fade in active");
                }
              ...
              $module.transition({
                ...
                onStart: function () {
                  if (
                    ...
                    (hideOthersToo ? $allModals : $previousModal)
                      .find(selector.dimmer)
                      .removeClass("in")
                      .addClass("out");
                  }
        ...
        set: {
          ...
          screenHeight: function () {
            if (module.can.fit()) {
              ...
              $context.css(
                "height",
                module.cache.height + settings.padding * 2 + "px",
              );
            }
          },
          active: function () {
  ...
  $.fn.modal.settings.templates = {
    ...
    prompt: function () {
      ...
      if (input.length === 0) {
        args.content +=
          '<p><div class="' +
          this.helpers.deQuote(settings.className.prompt) +
          '"><input placeholder="' +
