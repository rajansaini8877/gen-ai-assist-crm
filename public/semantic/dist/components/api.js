    ...
    $allModules.each(function () {
      var settings = $.isPlainObject(parameters)
        ...
        $context = settings.stateContext
          ? contextCheck(settings.stateContext, window)
          : $module,
        // request details
        ajaxSettings,
        requestSettings,
      ...
      module = {
        ...
        read: {
          cachedResponse: function (url) {
            ...
            response = sessionStorage.getItem(
              url + module.get.normalizedData(),
            );
          ...
          formData: function (data) {
            ...
            if (useFormDataApi) {
              ...
              settings.processData =
                settings.processData !== undefined
                  ? settings.processData
                  : false;
              ...
              $.each(formArray, function (i, el) {
                ...
                var isCheckbox =
                    $('[name="' + el.name + '"]', $form).attr("type") ===
                    "checkbox",
                  floatValue = parseFloat(el.value),
                  value =
                    (isCheckbox && el.value === "on") ||
                    el.value === "true" ||
                ...
                while (nameKeys.length > 0) {
                  ...
                  } else if (regExp.fixed.test(k)) {
                    // foo[n]
                    value = build([], k, value);
                  } else if (regExp.named.test(k)) {
                    // foo; foo[bar]
                    value = build({}, k, value);
                  }
                }
        ...
        get: {
          normalizedData: function () {
            return typeof settings.data === "string"
              ? settings.data
              : JSON.stringify(
                  settings.data,
                  Object.keys(settings.data).sort(),
                );
          },
