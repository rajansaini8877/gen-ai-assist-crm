...
(function ($, window, document) {
      ...
      module = {
        ...
        event: {
          field: {
            ...
            blur: function (event) {
              ...
              if (
                settings.on === "blur" ||
                (!$module.hasClass(className.initial) && settings.revalidate)
              ) {
        ...
        get: {
          ...
          changeEvent: function (type, $input) {
            return ["file", "checkbox", "radio", "hidden"].indexOf(type) >= 0 ||
              $input.is("select")
              ? "change"
              : "input";
          },
          ...
          prompt: function (rule, field) {
            ...
            if (
              ancillary &&
              ["integer", "decimal", "number", "size"].indexOf(ruleName) >= 0 &&
              ancillary.indexOf("..") >= 0
            ) {
            ...
            if (ancillary && ["match", "different"].indexOf(ruleName) >= 0) {
              prompt = prompt.replace(
                /{ruleValue}/g,
                module.get.fieldLabel(ancillary, true),
              );
            }
            if (requiresValue) {
        ...
        add: {
          ...
          prompt: function (identifier, errors, internal) {
            ...
            var $field = module.get.field(identifier),
              ...
              canTransition =
                settings.transition && module.can.useElement("transition");
        ...
        validate: {
          ...
          field: function (field, fieldName, showErrors) {
            ...
            if (!field) {
              module.debug(
                "Unable to find field validation. Skipping",
                fieldName,
              );
            ...
            if (isDisabled) {
              ...
              $.each(field.rules, function (index, rule) {
                if (
                  ...
                  (!errorLimit || fieldErrors.length < errorLimit)
                ) {
                  var invalidFields =
  ...
  $.fn.form.settings = {
    ...
    regExp: {
      ...
      decimal: /^\d+\.?\d*$/,
      email:
        /^[\w!#$%&'*+./=?^`{|}~-]+@[\da-z]([\da-z-]*[\da-z])?(\.[\da-z]([\da-z-]*[\da-z])?)*$/i,
      escape: /[$()*+,./:=?@[\\\]^{|}-]/g,
      flags: /^\/(.*)\/(.*)?/,
      integer: /^-?\d+$/,
      number: /^-?\d*(\.\d+)?$/,
    ...
    selector: {
      ...
      clear: ".clear",
      field:
        'input:not(.search):not([type="reset"]):not([type="button"]):not([type="submit"]), textarea, select',
    ...
    rules: {
      ...
      minLength: function (value, minLength) {
        return $.fn.form.settings.rules.range(
          value,
          minLength + "..",
          "integer",
          true,
