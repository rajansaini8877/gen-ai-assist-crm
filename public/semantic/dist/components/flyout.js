...
(function ($, window, document) {
    ...
    $allModules.each(function () {
      ...
      module = {
        initialize: function () {
          ...
          if (module.has.configActions()) {
            ...
            settings.actions.forEach(function (el) {
              var icon = el[fields.icon]
                  ? "<i " +
                    (el[fields.text] ? 'aria-hidden="true"' : "") +
                    ' class="' +
          ...
          if (
            module.get.direction() === "left" ||
            module.get.direction() === "right"
          ) {
        ...
        create: {
          flyout: function () {
            ...
            $module = $("<div/>", {
              class: className.flyout,
              role: "dialog",
              "aria-modal": settings.dimPage,
            });
            ...
            if (settings.title !== "") {
              ...
              $("<div/>", { class: className.header, id: titleId }).appendTo(
                $module,
              );
            }
            if (settings.content !== "") {
              ...
              $("<div/>", { class: className.content, id: descId }).appendTo(
                $module,
              );
            }
            if (module.has.configActions()) {
        ...
        event: {
          ...
          click: function (event) {
            if (
              ...
              settings.autofocus &&
              settings.dimPage &&
              $(document.activeElement).closest(selector.flyout).length === 0
            ) {
          ...
          closeKeyUp: function (event) {
            var keyCode = event.which;
            if (
              keyCode === settings.keys.enter ||
              keyCode === settings.keys.space
            ) {
              module.hide();
            }
          },
          ...
          approve: function (event) {
            if (
              ...
              settings.onApprove.call(module.element, $(this)) === false
            ) {
              module.verbose(
          ...
          deny: function (event) {
            if (
              ignoreRepeatedEvents ||
              settings.onDeny.call(module.element, $(this)) === false
            ) {
        ...
        bind: {
          ...
          events: function () {
            ...
            $context.on("click" + elementNamespace, module.event.click);
          },
          clickaway: function () {
            ...
            observer = new MutationObserver(function (mutations) {
              var collectNodes = function (parent) {
                  ...
                  for (var c = 0, cl = parent.length; c < cl; c++) {
                    Array.prototype.push.apply(
                      nodes,
                      collectNodes(parent[c].childNodes),
                    );
                    nodes.push(parent[c]);
                  }
              ...
              mutations.every(function (mutation) {
                if (mutation.type === "attributes") {
                  ...
                  var $addedInputs = $(collectNodes(mutation.addedNodes))
                      .filter("a[href], [tabindex], :input:enabled")
                      .filter(":visible"),
                    $removedInputs = $(
                      collectNodes(mutation.removedNodes),
                    ).filter("a[href], [tabindex], :input");
                  if ($addedInputs.length > 0 || $removedInputs.length > 0) {
                    shouldRefreshInputs = true;
                    if (
                      $addedInputs.filter(":input").length > 0 ||
                      $removedInputs.filter(":input").length > 0
                    ) {
        ...
        refreshInputs: function (ignoreAutofocus) {
          ...
          $inputs = $module
            .find("a[href], [tabindex], :input:enabled")
            .filter(":visible")
            .filter(function () {
        ...
        othersAnimating: function () {
          return (
            $flyouts.not($module).filter("." + className.animating).length > 0
          );
        },
        othersVisible: function () {
          return (
            $flyouts.not($module).filter("." + className.visible).length > 0
          );
        },
        ...
        set: {
          ...
          bodyMargin: function () {
            ...
            $context.css(
              (isBody ? "margin-" : "padding-") + position,
              tempBodyMargin + "px",
            );
            $context
        ...
        save: {
          ...
          bodyMargin: function () {
            initialBodyMargin = $context.css(
              (isBody ? "margin-" : "padding-") +
                (module.can.leftBodyScrollbar() ? "left" : "right"),
            );
            var bodyMarginRightPixel = parseInt(
                initialBodyMargin.replace(/[^\d.]/g, ""),
                10,
              ),
              bodyScrollbarWidth = isBody
                ? window.innerWidth - document.documentElement.clientWidth
                : $context[0].offsetWidth - $context[0].clientWidth;
  ...
  $.fn.flyout.settings = {
    ...
    selector: {
      bodyFixed:
        "> .ui.fixed.menu, > .ui.right.toast-container, > .ui.right.sidebar, > .ui.right.flyout, > .ui.fixed.nag, > .ui.fixed.nag > .close",
      fixed: ".fixed",
    ...
    error: {
      ...
      movedFlyout:
        "Had to move flyout. For optimal performance make sure flyout and pusher are direct children of your body tag",
